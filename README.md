...
C的内部函数的是静态的，在编译后，函数的内存地址就确定了。但是，外部的函数地址在执行前是不能确定的，也就是说C的底层也有动态的。`Fishhook`之所以能 hook C系统函数，就是利用了 Mach-O 文件的 `PIC` 技术特点，**`将指向系统函数的符号重新进行绑定指向内部的函数`**。这样就把系统方法与自己定义的方法进行了交换。
