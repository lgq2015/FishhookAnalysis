...
所以说，**`C的底层也有动态的表现`**。C在内部函数的时候是静态的，在编译后，函数的内存地址就确定了。但是，外部的函数是不能确定的，也就是说C的底层也有动态的。fishhook 之所以能 hook C函数，就是利用了 Mach-O 文件的 `PIC` 技术特点。也就造就了静态语言C也有动态的部分，通过 DYLD 进行动态绑定的时候做了手脚。**`但是相反的，fishhook是hook不了内部的C函数的`**。
